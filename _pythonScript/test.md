    ...
    try {
        ...
        for (std::vector<std::string>::iterator it = messages.begin(); it != messages.end(); ++it) {
            ...
            if (!client->isRegistered() && (commandName == "NICK" || commandName == "USER")) {
                ...
                ClientManager::getInstance().sendMessageToClient(clientSocket, "ERROR: Registration required\n");
                close(clientSocket);
...
void IRCServer::handleRegistration(int clientSocket, const std::string& commandName, const std::vector<std::string>& args) {
    ...
    if (commandName == "NICK" || commandName == "USER") {
...
Client::Client(int socket) : clientSocket(socket), registered(false) {
...
// PING command class implementation
void PingCommand::execute(int clientSocket, const std::vector<std::string>& args) {
    if (args.size() < 2) {
        ClientManager::getInstance().sendMessageToClient(clientSocket, "ERROR: PING command requires a parameter\n");
        return;
    }
